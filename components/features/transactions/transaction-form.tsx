...
import { MAX_TAGS_PER_TRANSACTION, MAX_TAG_LENGTH } from "@/lib/tag-helpers";
...
export function TransactionForm({
  ...
  const [showSoftClosedConfirm, setShowSoftClosedConfirm] =
  ...
  const calculatedExchangeRate =
    isDualCurrency &&
    parseFloat(amountBase) > 0 &&
    parseFloat(amountSecondary) > 0
  ...
  const handleAddTag = (name: string, existingId?: string) => {
    ...
    if (
      !availableTags.some((t) => t.name.toLowerCase() === trimmed.toLowerCase())
    ) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const hasSecondaryAmount =
      amountSecondary && parseFloat(amountSecondary) > 0 ? true : false;
    const hasSecondaryCurrency =
      currencySecondary && currencySecondary.trim() !== "" ? true : false;

    if (hasSecondaryAmount !== !!hasSecondaryCurrency) {
      toast.error(
        "Please provide both secondary amount and currency, or neither"
      );
      return;
    }
      ...
      const response = await fetch(
          ...
          : `/api/orgs/${orgSlug}/transactions`,
        {
          ...
          body: JSON.stringify({
            ...
            amountSecondary: hasSecondaryAmount
              ? parseFloat(amountSecondary)
              : null,
  ...
  return (
    <>
      <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
        ...
        {isInSoftClosedPeriod && (
          <Alert>
            ...
            <AlertDescription>
              This transaction is POSTED in a soft-closed period. Editing it may
              affect previously reported figures. You will be asked to confirm
        ...
        <div className="space-y-2">
          <Label htmlFor="amountBase">
            Base Amount ({settings.baseCurrency}){" "}
        ...
        <div className="space-y-2">
          <Label htmlFor="amountSecondary">Original Amount (optional)</Label>
          <Input
            id="amountSecondary"
            type="number"
            step="0.01"
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) =>
              setCurrencySecondary(value === "none" ? "" : value)
            }
            disabled={isLoading}
          >
          ...
          {calculatedExchangeRate && (
            <p className="text-xs text-muted-foreground">
              Exchange rate: 1 {currencySecondary} = {calculatedExchangeRate}{" "}
              {settings.baseCurrency}
            </p>
          )}
        </div>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex flex-col gap-2 sm:flex-row">
              <Input
                ...
                disabled={
                  isLoading || selectedTags.length >= MAX_TAGS_PER_TRANSACTION
                }
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    e.preventDefault();
            ...
            <p className="text-xs text-muted-foreground">
              Up to {MAX_TAGS_PER_TRANSACTION} tags per transaction. New tags
      ...
      <Dialog
        open={showSoftClosedConfirm}
        onOpenChange={setShowSoftClosedConfirm}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
            ...
            <DialogDescription>
              This transaction is POSTED in a soft-closed period. Changing it
              may alter previously reported figures.
              <br />
              <br />
