...
### 7.1 Base Currency & Secondary Currency

- Each business has a **Base Currency** (e.g. MYR) set during onboarding.
- Each transaction has:
  - Base currency amount (required, positive number only).
  - Base currency code (required, from business settings).
  - Secondary currency amount (optional, positive number only).
  - Secondary currency code (optional, required if secondary amount is provided).

**Use case:**

- User's base currency is MYR, but they deal with USD transactions.
- For a USD transaction, they enter:
  - Secondary: USD 250.00 (original transaction amount).
  - Base: MYR 1,000.00 (accounting amount for reporting).

- The base currency amount is used for all reporting, calculations, filtering, searching, and sorting.

### 7.2 Dual Currency Display

**Transaction list and detail views:**

- For transactions with secondary currency:
  - Display format (stacked, base currency prominent):
    - Primary line: MYR 1,000.00 (base currency, normal size).
    - Secondary line: USD 250.00 (smaller text, displayed below base).

- For base-currency-only transactions:
  - Display only base currency amount and code.
  - Secondary currency shows as "N/A" or is hidden entirely.

### 7.3 Secondary Currency Validation

**Rules:**

- **Base currency:**
  - Amount and code are always required for every transaction.
  - Code is set automatically from business settings.

- **Secondary currency (all-or-nothing):**
  - Both amount and code must be provided together.
  - If user provides secondary amount, secondary code is required.
  - If user provides secondary code, secondary amount is required.
  - If neither is provided, transaction is base-currency-only.

- **Transaction origin:**
  - If secondary currency exists, the secondary currency is assumed to be the original transaction currency.
  - No explicit tracking field needed.

### 7.4 Currency Code Selection

**Input mechanism:**

- Currency codes are selected via **searchable dropdown**:
  - Includes common currencies: USD, MYR, EUR, GBP, SGD, JPY, AUD, CNY, THB, HKD, etc.
  - User can type to filter the list for quick selection.
  - Validates against ISO 4217 currency code standard.
  - Prevents invalid or misspelled codes.

**Storage:**

- Currency codes are stored as **plain text** (3-letter ISO codes) in the transaction table.
- No separate Currency lookup table needed.

### 7.5 Filtering, Searching, and Sorting

**Base currency only:**

- All transaction operations use base currency amount exclusively:
  - Filtering by amount range.
  - Searching by amount.
  - Sorting by amount.
  - Reports and dashboards.
  - Calculations (totals, P&L, balances).

- Secondary currency is for **display and reference purposes only**.

### 7.6 Migration from Exchange Rate Model
...
- **Migration strategy:**
  - For each transaction:
    - If `currencyOriginal` ≠ base currency:
      - Move `amountOriginal` → `amountSecondary`
      - Move `currencyOriginal` → `currencySecondary`
      - Keep `amountBase` → `amountBase`
      - Set `currencyBase` from business settings.

    - If `currencyOriginal` = base currency:
      - Move `amountOriginal` → `amountBase`
      - Set `currencyBase` from business settings.
      - Leave `amountSecondary` and `currencySecondary` as null.

  - Remove exchange rate fields after migration:
    - `exchangeRateToBase`
    - `rateSource` (if exists)
    - `rateTimestamp` (if exists)
    - Any manual rate override fields
